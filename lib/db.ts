...
function getSql() {
    if (!_sql) {
          _sql = postgres(process.env.POSTGRES_URL!, { ssl: "require", max: 5 });
    }
    return _sql;
}
...
async function pgRemove(table: Table, id: string): Promise<boolean> {
    ...
    const rows =
          table === "wisuda"
        ? await sql`DELETE FROM wisuda WHERE id = ${id} RETURNING id`
            : await sql`DELETE FROM pmb WHERE id = ${id} RETURNING id`;
